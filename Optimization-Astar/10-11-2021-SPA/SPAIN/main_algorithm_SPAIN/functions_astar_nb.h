...
int binary_reader(node *nodes){ 
        FILE *graph;
        unsigned long number_nodes =  23895681, ntotnsucc = 25394533;
        unsigned long *allsuccessors;
        char *allnames;
        
        if ((graph = fopen (inputfilename, "r")) == NULL)
        ...
        if( ( allnames = (char *) malloc(ntotnsucc* sizeof(char))) == NULL)
            ExitError("when allocating memory for the edges vector", 15);
        ...
        if(fread(allnames, sizeof(char), ntotnsucc, graph) != ntotnsucc)
            ExitError("when reading sucessors from the binary data file", 18);
        ...
        for(long unsigned int i=0; i < number_nodes; i++) if(nodes[i].numbersegments) {
            nodes[i].name = allnames; allnames += nodes[i].numbersegments;
