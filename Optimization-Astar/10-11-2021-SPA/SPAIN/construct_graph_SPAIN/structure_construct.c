...
int main(){
    ...
    char *data_string, *token, *row, *oneway, *streetname;
            ...
            while ( (token = strsep(&row, "|")) != NULL){
                ...
                if (nextnodeposition == -1){continue;}
                    ...
                    if (strcmp(oneway,"oneway")){
                        ...
                        if(streetname==NULL){
                            if((nodes[actualnodeposition].name = (char *)realloc(nodes[actualnodeposition].name,((nodes[actualnodeposition].numbersegments)+1)* sizeof(char))) == NULL){
                            printf("Can't allocate more memory for the succesors");
                            return 4;
                            }
                            nodes[actualnodeposition].name = " ";
                        } 
                        else{
                            if((nodes[actualnodeposition].name = (char *)realloc(nodes[actualnodeposition].name,((nodes[actualnodeposition].numbersegments)+1)* sizeof(char))) == NULL){
                                printf("Can't allocate more memory for the succesors");
                            ...
                            nodes[actualnodeposition].name[nodes[actualnodeposition].numbersegments] = *streetname ;
                        }
                        ...
                        if(streetname==NULL){
                            if((nodes[actualnodeposition].name = (char *)realloc(nodes[actualnodeposition].name,((nodes[actualnodeposition].numbersegments)+1)* sizeof(char))) == NULL || 
                                (nodes[nextnodeposition].name = (char *)realloc(nodes[nextnodeposition].name,((nodes[nextnodeposition].numbersegments)+1)* sizeof(char))) == NULL){
                            printf("Can't allocate more memory for the succesors");
                            return 4;
                            }
                            nodes[actualnodeposition].name = " ";
                            nodes[nextnodeposition].name = " ";
                        } 
                        else{
                            if((nodes[actualnodeposition].name = (char *)realloc(nodes[actualnodeposition].name,((nodes[actualnodeposition].numbersegments)+1)* sizeof(char))) == NULL || 
                                (nodes[nextnodeposition].name = (char *)realloc(nodes[nextnodeposition].name,((nodes[nextnodeposition].numbersegments)+1)* sizeof(char))) == NULL){
                            ...
                            nodes[actualnodeposition].name[nodes[actualnodeposition].numbersegments] = *streetname ;
                            nodes[nextnodeposition].name[nodes[nextnodeposition].numbersegments] = *streetname ;
                        }
    ...
    for(int i=0; i < number_nodes; i++) if(nodes[i].numbersegments) {
        ...
        if( fwrite(nodes[i].name, sizeof(char), nodes[i].numbersegments, fin) != nodes[i].numbersegments)
            ExitError("when writing edges to the output binary data file", 32);
    }
