...
#define outputfilename "Catalunya.bin"
#define inputfilename "/home/joseba/Master/Data/cataluna.csv"
...
int main(){
    ...
    node *nodes; 
    size_t row_size = 0;
    int node_num = 0, way_num=0, actualnodeposition, nextnodeposition;
    char *data_string, *token, *row, *oneway;
    long unsigned int number_nodes =  3472620, streetid, actualnodeid, nextnodeid;
    ...
    // printf("number of nodes ,%d",node_num);
    // /* Then we print the data */
    // printf("\nPrinting the nodes:\n");
    // for (i = 0; i < 10; i++){
    //     printf("\nId: %lu Nam: %s Latitutde: %lf Longitude: %lf Nsuccesors: %d\n", nodes[i].id, nodes[i].name, nodes[i].latitude, nodes[i].longitude, nodes[i].numbersegments);
